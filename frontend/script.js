...
async function openProdottoModal(prodotto = null) {
  ...
  selectMarca.innerHTML =
    ...
    marche.map((m) => `<option value="${m.id}">${m.nome.toUpperCase()}</option>`).join("")
        ...
        <div class="prodotto-block">
          <div class="prodotto-header">
            <div class="prodotto-info">
              ...
              <span><strong>Giacenza Totale:</strong> ${prodotto.giacenza} pz</span>
            </div>
            <div class="prodotto-info">
...
async function loadStorico() {
  ...
  try {
    ...
    document.getElementById("valoreStorico").textContent = `€ ${Number.parseFloat(result.valore_totale || 0).toFixed(
      2,
    )}`
  ...
  storico.forEach((s) => {
    ...
    <tr class="product-main-row">
      <td><strong>${s.nome}</strong>${
        s.marca_nome ? ` <span class="badge-marca">(${s.marca_nome.toUpperCase()})</span>` : ""
      }</td>
      <td>${
        s.descrizione
