...
router.post("/", (req, res) => {
  ...
  db.run("INSERT INTO marche (nome, data_creazione) VALUES (?, ?)", [nome.trim(), data_creazione], function (err) {
    if (err) {
...
router.put("/:id", (req, res) => {
  ...
  db.run("UPDATE marche SET nome = ? WHERE id = ?", [nome.trim(), id], function (err) {
    if (err) {
      if (err.message.includes("UNIQUE")) {
        return res.status(400).json({ error: "Marca già esistente" })
      }
...
router.delete("/:id", (req, res) => {
  ...
  db.get("SELECT COUNT(*) as count FROM prodotti WHERE marca_id = ?", [id], (err, row) => {
    if (err) {
      return res.status(500).json({ error: err.message })
    }
    ...
    if (row.count > 0) {
      return res.status(400).json({
        error: `Impossibile eliminare: ci sono ${row.count} prodott${row.count === 1 ? "o" : "i"} collegat${row.count === 1 ? "o" : "i"} a questa marca.`,
      })
    }
